...
def _sequence_event(values, length, verb):
  ...
  event_description = 'последовательности {sequence}'.format(sequence=sequence)
  return event, event_description


def _word_series(words, conjunction='и'):
...
def _swr_space(is_train, sample_range):
  ...
  ending = ''
  if re.fullmatch('\d*(?<!1)1', str(num_sampled)):
    ending += 'а'
  elif re.fullmatch('\d*(?<!1)[234]', str(num_sampled)):
    ending += 'ы'


  random_variable.description = (
      str(display.StringNumber(num_sampled))
      + ' букв{} выбраны без возвращения из '.format(ending)
      + sample.bag_contents)
...
def _sample_without_replacement_probability_question(
  ...
  while True:
    ...
    event, event_description = event_fn(
        values=distinct_letters, length=space.n_samples, verb='выбрать')
